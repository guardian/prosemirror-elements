...
>(
  ...
  node.forEach((node) => {
    ...
    const value = getFieldValueFromNode(node, fieldDescription, serializer, elementTypeMap);
...
>(
  ...
  if (fieldDescription.type === "repeater") {
    ...
    node.forEach((childNode) => {
      values.push(
        getFieldValuesFromNode(childNode, fieldDescription.fields, serializer, elementTypeMap)
      );
    });
    return values;
  }
  if (fieldDescription.type === "nestedElement") {
    return getValuesFromNestedElementContentNode(node, serializer, elementTypeMap);
  }
  return undefined;
};
...
>(
  ...
  const nestedElements: ExtractDataTypeFromElementSpec<ESpecMap, Extract<keyof ESpecMap, string>>[] = [];
  if (elementTypeMap){
    node.forEach((childElement) => {
      const elementData = createGetElementDataFromNode(elementTypeMap)(childElement, serializer)
      if (elementData){
